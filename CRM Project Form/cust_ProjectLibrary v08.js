/*
	...
	Adam Ip																	2014-09-23	
	function ProjectForm_OnSave()

	Adam Ip																	2014-11-06	
	function EnableSignedSOWReceived()
...
/***************************************************************************************/
/*  To enable or disable the data field Signed SOW Received 
    Call IsSignedSOWRecevied() to determine whether the current user belongs to 
		accessible or not? read-only user group? read-write user group?		
*/
function EnableSignedSOWReceived()
{
...
/***************************************************************************************/
function ValidateEndUserContact()
{
	if( DebugModeProject ) window.alert( "ValidateEndUserContact" );

	try
	{
		/* End User Account */
		var oCheck = myGetValue( "new_projectenduser" );
		
		if( DebugModeProject ) window.alert( "ValidateEndUserContact\noCheck = " + oCheck );
		if( oCheck != null )	/* End User Account */
		{
			var oFill = myGetValue( "new_endusercontact" );
			if( DebugModeProject ) window.alert( "ValidateEndUserContact\noCheck = " + oCheck + "\noFill = " + oFill );
			if( oFill == null ) 
			{
				mySetFocus( "new_endusercontact" );
				window.alert( "You must fill in End User Contact." );
	...
	{
		if( DebugModeProject ) window.alert( "function ValidateEndUserContact error code " + err );
	}
}
...
function ProjectForm_OnSave()
{
	if( DebugModeProject ) window.alert( "function ProjectForm_OnSave" );
	try
	{
		/* ValidateEndUserContact(); */		
	}
	catch( err )
	{
		if( DebugModeProject ) window.alert( "function ProjectForm_OnSave error code " + err );
	}
}
